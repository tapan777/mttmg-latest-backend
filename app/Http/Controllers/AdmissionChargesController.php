...
use App\Models\AdmissionCharges;
...
class AdmissionChargesController extends Controller
{
    ...
    public function createadmissionValue(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $package_details['package_type'] = 1;
    ...
    {
        $model = Package::find($request->admission_value_id);
        if ($model) {
            // Only mass-assign model fillable fields; ignore client extras (e.g. auth_user_id from interceptors).
    ...
    public function retriveAdmissionValue(Request $request)
    {
        ...
        $package_names = Package::get(['id', 'name']);
        $load_data = DB::table('packages')
        ->where('package_type', 1) // Apply the `package_type = 1` condition
    ...
    {
        $package_details = Package::where('status', 1)
        ->where('package_type', 1) // Add the condition for package_type
        ->get(['id', 'name']);
    ...
    {
        $package_details = Package::find($request->id);
        if ($package_details) {
            return response()->json([
         ...
         public function autoComplete(Request $request)
         {
             ...
             ->where('package_type', 1) // Apply the `package_type` condition
             ->where(function ($query) use ($search_text) {
                 $query->where('name', 'like', "%$search_text%")
                       ->orWhere('duration', 'like', "%$search_text%");
             })
